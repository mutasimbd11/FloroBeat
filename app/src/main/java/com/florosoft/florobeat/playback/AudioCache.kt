 ...
 * original `florobeat://watch?v=<id>` request, and a cache hit never resolves a
 * URL at all.
 */
@UnstableApi
object AudioCache {
     ...
     * [StreamResolver][com.florosoft.florobeat.data.innertube.StreamResolver]'s
     ...
     * `florobeat://watch?v=…` URI it builds for itself carries an id and nothing
     * else. It rides along from the queue instead — see
     * [PlaybackService.prefetchAround][com.florosoft.florobeat.playback.PlaybackService].
     */
     ...
     * [PlaybackService][com.florosoft.florobeat.playback.PlaybackService]'s audition.
    ...
    private suspend fun fetch(videoId: String, position: Long, length: Long) =
        fetch(videoId, Uri.parse("florobeat://watch?v=$videoId"), position, length)
