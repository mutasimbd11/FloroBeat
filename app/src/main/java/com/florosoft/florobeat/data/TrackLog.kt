 ...
 * FloroBeat to access all device logs?"* — which appears whenever the process
 ...
 * to logcat as well, so `adb logcat -s FloroBeat` is unchanged.
 */
object TrackLog {
    ...
    private fun header(
        ...
        appendLine("FloroBeat log — ${song.title} — ${song.artist}")
