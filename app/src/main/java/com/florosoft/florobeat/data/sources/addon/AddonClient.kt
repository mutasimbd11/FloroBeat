package com.florosoft.florobeat.data.sources.addon

import com.florosoft.florobeat.data.Http
import com.florosoft.florobeat.data.TrackLog
import com.florosoft.florobeat.data.sources.module.SharedCalls
 ...
 * this side. Compare [ModuleManager][com.florosoft.florobeat.data.sources.module.ModuleManager],
...
class AddonClient(rawBaseUrl: String) {
    ...
    suspend fun manifest(): Result<AddonManifest> = manifests.get(
        ...
        fetch<AddonManifest>(manifestUrl(baseUrl)).mapCatching { manifest ->
            ...
            if (!manifest.isPlayable) {
                ...
                throw AddonException("This addon declares $declared — FloroBeat needs search")
            }
            manifest
        ...
        while (true) {
            ...
            val wait = client.newCall(request).execute().use { response ->
                when {
                    ...
                    attempt >= MAX_RETRIES -> throw AddonUnavailable("This addon is rate limiting FloroBeat")
    ...
    companion object {
        ...
        /** The tiers [AddonSource][com.florosoft.florobeat.data.sources.AddonSource] asks in. */
        ...
        private const val USER_AGENT = "FloroBeat"
 ...
 * [SourceHealth.Rejected][com.florosoft.florobeat.data.sources.SourceHealth.Rejected],
 ...
 * [SourceHealth][com.florosoft.florobeat.data.sources.SourceHealth] draws.
 */
