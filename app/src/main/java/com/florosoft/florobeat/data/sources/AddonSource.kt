...
import com.florosoft.florobeat.data.sources.addon.AddonNotFound
import com.florosoft.florobeat.data.sources.addon.AddonStream
import com.florosoft.florobeat.data.sources.addon.AddonTrack
import com.florosoft.florobeat.playback.StreamContainer
...
class AddonSource(
     ...
     * [QualityUpgrade][com.florosoft.florobeat.playback.QualityUpgrade] checks a
     ...
     * [TrackMatcher][com.florosoft.florobeat.data.sources.TrackMatcher] decides which
    ...
    private fun openable(url: String, answer: AddonStream, trackId: String, tier: String): SourceStream? {
        ...
        if (answer.isEncrypted) {
            TrackLog.w(
                TAG,
                "${config.displayName}: $trackId came back encrypted, which FloroBeat never asked for — passing",
            )
            return null
        }
