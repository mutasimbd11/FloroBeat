package com.florosoft.florobeat.data.stats
...
object Backup {
    ...
    fun suggestedName(): String =
        "florobeat-backup-${DateTimeFormatter.ofPattern("yyyy-MM-dd").format(
    ...
    suspend fun importFrom(context: Context, source: Uri): Result<Summary> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val file = runCatching { json.decodeFromString(BackupFile.serializer(), text) }
                .getOrElse { error("That doesn't look like a FloroBeat backup") }
            require(file.app == APP_TAG) { "That backup is from another app" }
            require(file.version <= SCHEMA_VERSION) {
                "That backup was written by a newer version of FloroBeat"
            }
    ...
    private const val APP_TAG = "FloroBeat"
