package com.florosoft.florobeat.data.settings
...
import com.florosoft.florobeat.playback.EqLayout
import com.florosoft.florobeat.playback.EqualizerPreset
...
enum class AudioQuality(
     ...
     * [SourceConfig.enabled][com.florosoft.florobeat.data.sources.SourceConfig.enabled],
 ...
 * Opus-in-WebM (see
 * [StreamResolver.resolveForDownload][com.florosoft.florobeat.data.innertube.StreamResolver.resolveForDownload]),
...
object AppSettings {
    ...
    /**
     * Keep ordinary downloads in Music/FloroBeat where other music apps can see
    ...
    // [SourceResolver.requestForNow][com.florosoft.florobeat.data.sources.SourceResolver.requestForNow],
     ...
     * decode per track.
     *
     * See [com.florosoft.florobeat.playback.smart.TransitionPlanner].
     */
     ...
     * [ModuleSource.unplayable][com.florosoft.florobeat.data.sources.ModuleSource],
     * which is where the refusal is applied.
     *
     * Independent of whether the device *can* decode it — that question is
     * [DeviceCodecs.playsDolbyAtmos][com.florosoft.florobeat.data.sources.DeviceCodecs],
    ...
    /**
     * Widens stereo output via [com.florosoft.florobeat.playback.SpatialAudioProcessor],
    ...
    /** Requests a sustained high-refresh UI for smooth scrolling. Enabled by default. */
    val highPerformanceMode = MutableStateFlow(true)
     ...
     * all. See [CanvasRepository][com.florosoft.florobeat.data.canvas.CanvasRepository].
     */
     ...
     * — see [CanvasCache][com.florosoft.florobeat.data.canvas.CanvasCache] for why
     ...
     * [NowPlayingScreen][com.florosoft.florobeat.ui.player.NowPlayingScreen].
     */
     ...
     * that have not gone away — see [ArtworkMesh][com.florosoft.florobeat.ui.player.ArtworkMesh]
     * for the colour one (a cover that is nine-tenths black with a red stripe
     * comes back from the quantiser as a red screen) and
     * [ArtworkMeshBackdrop][com.florosoft.florobeat.ui.player.ArtworkMeshBackdrop]
    ...
    /**
     * The order [lyricsSources] are asked in — see [LyricsRepository][com.florosoft.florobeat.data.lyrics.LyricsRepository]:
     ...
     * [ArtistFacts][com.florosoft.florobeat.data.stats.ArtistFacts]. On by default,
     ...
     * [PlaybackService][com.florosoft.florobeat.playback.PlaybackService] can pick
    ...
    fun init(context: Context) {
        prefs = context.getSharedPreferences("florobeat_settings", Context.MODE_PRIVATE)
     ...
     *
     * The one caller is an import ([Backup][com.florosoft.florobeat.data.stats.Backup]),
    ...
    private fun readAll() {
        ...
        com.florosoft.florobeat.data.lyrics.PaxSenix.setApiKey(paxSenixApiKey.value)
     ...
     *
     * FloroBeat ships sideloaded (see [com.florosoft.florobeat.data.AppUpdateChecker]),
    ...
    fun recordDismissedUpdate(version: String) {
        prefs.edit()
    ...
    fun isUpdateDismissed(version: String): Boolean {
        val dismissedVer = prefs.getString(KEY_DISMISSED_UPDATE_VERSION, null) ?: return false
        if (dismissedVer != version) return false
        val dismissedTime = prefs.getLong(KEY_DISMISSED_UPDATE_TIME, 0L)
        return System.currentTimeMillis() - dismissedTime < com.florosoft.florobeat.data.update.UpdateConfig.DISMISSED_REMIND_COOLDOWN_MS
    }

    fun clearDismissedUpdate() {
        prefs.edit()
            .remove(KEY_DISMISSED_UPDATE_VERSION)
            .remove(KEY_DISMISSED_UPDATE_TIME)
            .apply()
    }

    fun recordSuccessfulUpdateCheck() {
        prefs.edit()
            .putLong(KEY_LAST_UPDATE_CHECK_TIME, System.currentTimeMillis())
            .apply()
    }

    fun isUpdateCheckCooldownActive(): Boolean {
        val lastCheck = prefs.getLong(KEY_LAST_UPDATE_CHECK_TIME, 0L)
        return System.currentTimeMillis() - lastCheck < com.florosoft.florobeat.data.update.UpdateConfig.BACKGROUND_CHECK_COOLDOWN_MS
    }
    ...
    fun setPaxSenixApiKey(value: String) {
        val normalized = com.florosoft.florobeat.data.lyrics.normalizePaxSenixApiKey(value)
        paxSenixApiKey.value = normalized
        prefs.edit().putString(KEY_PAXSENIX_API_KEY, normalized).apply()
        com.florosoft.florobeat.data.lyrics.PaxSenix.setApiKey(normalized)
    }
     ...
     *
     * [Downloads][com.florosoft.florobeat.download.Downloads] keeps its record of
    ...
    private const val KEY_DISMISSED_UPDATE_TIME = "dismissed_update_time"
