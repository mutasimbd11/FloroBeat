...
private fun drawHeader(
    ...
    canvas.drawText("FloroBeat", POSTER_W - MARGIN, 132f, brand)
    ...
    val wordWidth = brand.measureText("FloroBeat")
...
private fun drawFooter(canvas: Canvas, context: Context, type: Fonts) {
    canvas.drawText(
        context.getString(R.string.counted_on_device_with_florobeat),
        MARGIN,
        POSTER_H - 64f,
...
/**
 * The FloroBeat mark, baseline-aligned with the word beside it.
 *
...
private fun drawLogo(canvas: Canvas, context: Context, right: Float, baseline: Float) {
    val logo = runCatching {
        ResourcesCompat.getDrawable(context.resources, R.drawable.florobeat_foreground, null)
    }.getOrNull() ?: return
